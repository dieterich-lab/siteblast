...
"$Id: bz_extend.c,v 1.1 2004/05/05 15:36:24 mmichael Exp $";
...
{
	int i, *sp[NACHARS], numMSPs = MSP_TAB_NUM(mt);
	double scor;
	AlignIOPtr align_IO;
	...
	if (SEQ_LEN(sf1) == SEQ_LEN(sf2)) {
		...
		if (i == SEQ_LEN(sf1)) {   /* seq1 and seq2 are identical */
			...
			if (scor < (float)K)
				a->score = K;	// so it won't be discard
			else if (scor > (float)INT_MAX)
				a->score = INT_MAX;
			else
				a->score = scor;
			a->next_align = NULL;
