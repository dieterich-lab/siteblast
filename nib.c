...
"$Id: nib.c,v 4.0 2004/08/26 09:49:25 mmichael Exp mmichael $";
...
static const u32_t NIB_SIG = 0x6BE93D3A;
static const u32_t NIB_GIS = 0x3A3DE96B;
enum { NIB_MSK = 1U<<3 };
...
static u32_t getlen(FILE *fp, u32_t sig)
{
	u32_t a, b, c, d;
...
unsigned char *seq_freadnib(FILE *fp, i32_t rbase, i32_t rlen, i32_t *slen)
{
	u32_t sig, len, i;
	unsigned char *s;
...
unsigned char *seq_readnib(const char *fname, i32_t rbase, i32_t rlen, i32_t *slen)
{
...
void seq_fwritenib(FILE *fp, unsigned const char *s, u32_t len)
{
	u32_t i;
...
void seq_writenib(char *fname, unsigned const char *s, u32_t len)
{
