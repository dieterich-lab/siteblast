...
"$Id: bz_hit19.c,v 1.1 2004/05/05 15:36:24 mmichael Exp $";
...
//              abcdefghijklmnopqrs -> abcehilnpqrs
...
//              abcdefghijklmnopqr -> abcehjmnpqr
...
blast_table_t *blast_1219_new(SEQ *seq, int W)
{
	...
	W = 0; // because we don't add transitions to this table anymore.
...
//STATISTICS(STATS.extend_hit++);
			...
			// search transitions too
			if (T) for (i = 0; i < 12; ++i) {
			    unsigned int t;
			    t = ecode ^ (2 << (2 * i)); /* A~G; C~T */
			    for (h = bt->n.hashtab[t]; h; h = h->next) {
//STATISTICS(STATS.extend_hit++);
				msp_extend_hit(mt, seq1, seq2,
                                                ss, X, K, 19, P,
                                                h->pos, s-seq+1, diag_lev);
...
// ----------
// Note: a more general version of the transformation function:
//  j = (ecode & (3<<(2*i))) >> (2*i);
//  j = transition[j];
//  t = (ecode &~ (3<<(2*i))) | (j << (2*i));
// A simple version that just does transitions by flipping the right bit:
//  t = ecode ^ (2 << (2 * i)); /* A~G; C~T */
// ----------
